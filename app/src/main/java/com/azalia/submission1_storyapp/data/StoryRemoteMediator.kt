...
import com.azalia.submission1_storyapp.data.local.RemoteKeys
...
class StoryRemoteMediator (
    ...
    private suspend fun getRemoteKeyForLastTime(state: PagingState<Int, StoryEntity>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id)
        }
    }

    private suspend fun getRemoteKeyForFirstTime(state: PagingState<Int, StoryEntity>): RemoteKeys? {
